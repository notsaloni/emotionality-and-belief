...
# Filter for True (T) and False (F) headlines
df_true <- df %>% filter(grepl("T", headline_no))
df_false <- df %>% filter(grepl("F", headline_no))

View(df_false_neutral)

# Further split into Emotional (E) and Neutral (N) for True (T) and False (F)
df_false_emotional <- df_false %>% filter(grepl("E$", headline_no))
df_false_neutral <- df_false %>% filter(grepl("N$", headline_no))
